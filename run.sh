...
odin build $SCRIPT_DIR/journey_ecs.odin -file -debug -out:main-debug.bin
odin build $SCRIPT_DIR/journey_ecs.odin -file -o:size -microarch:native -out:main-release.bin
...
odin build $SCRIPT_DIR/journey_ecs.odin -file -debug -build-mode:llvm-ir

odin run $SCRIPT_DIR/journey_ecs.odin -file -debug -define:CSV_DUMP=true -define:CORE=$(getconf _NPROCESSORS_ONLN)

echo "✅ Build + disassembly complete. Output: assembly_dump.asm + assembly_dump-release.asm"

valgrind --tool=cachegrind --D1=$L1D_SIZE,$L1D_ASSOC,$L1D_LINE_SIZE --I1=$L1I_SIZE,$L1I_ASSOC,$L1I_LINE_SIZE --LL=$L3_SIZE,$L3_ASSOC,$L3_LINE_SIZE  --cache-sim=yes --branch-sim=yes --cachegrind-out-file=debug-cachegrind $SCRIPT_DIR/main-release.bin
